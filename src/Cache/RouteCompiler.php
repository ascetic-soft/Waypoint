...
final class RouteCompiler
{
     ...
     *
     * If the cache was generated by the current compiler version the trie is
     * restored directly — no sorting or trie construction at runtime.
    ...
    public function load(string $cacheFilePath): RouteCollection
    {
        ...
        // ── New format: routes + pre-built trie + fallback indices ──
        if (isset($data['trie'])) {
            /** @var list<array{path: string, methods: list<string>, handler: array{0:class-string,1:string}|\Closure, middleware?: list<string>, name?: string, compiledRegex: string, parameterNames: list<string>, priority?: int, argPlan?: list<array<string, mixed>>}> $routeItems */
            $routeItems = $data['routes'];
            /** @var array{static: array<string, mixed>, param: list<array<string, mixed>>, routes: list<int>} $trieData */
            $trieData = $data['trie'];
            /** @var list<int> $fallbackIndices */
            $fallbackIndices = $data['fallback'];

            $routes = [];
            foreach ($routeItems as $item) {
                $routes[] = Route::fromArray($item);
            }
            ...
            $trie = RouteTrie::fromArray($trieData, $routes);

            return RouteCollection::fromCompiled($routes, $trie, $fallback);
        }
